...
const ROUTES = [
  ...
  /* The dashboard is a topic area of the VSH program (PRJ-00), not a page in
     ...
     Loads every apiary/hive/inspection (Phase 5) and every queen line
     (Phase 4, for the "Contributing breeders" panel — a full roster,
     including lines with no hives yet, not just whatever's embedded
     per-hive) side by side. */
  { test: /^#\/projects\/p0\/dashboard\/?$/, view: renderDashboard,
    load: () => Promise.all([loadApiaries(), loadQueenLines()])
      .then(([ap, queenLines]) => ({ ...ap, queenLines })) },
