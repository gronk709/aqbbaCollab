/* ==========================================================================
   Collaborator invite callback.

   Mirrors js/waAuth.js's shape, but for Supabase's own email-invite links
   instead of Wild Apricot's OAuth redirect. supabase.auth.admin
   .inviteUserByEmail (see supabase/functions/invite-collaborator) sends an
   email whose link redirects back here with the new session's tokens in
   the URL HASH — #access_token=...&refresh_token=...&type=invite — which
   is Supabase's standard shape for every email-based auth link (magic
   link, invite, recovery), independent of anything set client-side.
...
/* Extracts the tokens and scrubs the hash so a reload doesn't try to
   ...
   on success, or { error } if the hash didn't actually carry what
   isInviteAuthCallback() expected. */
export function consumeInviteAuthCallback() {
  ...
  if (!access_token || !refresh_token) return { error: 'Invite link is missing its sign-in tokens.' };
